...
<?php 		
			if($do == 'detay') {
				$sayfa = (isset($q) ? $q : 1);
				$toplam_veri_sayisi = $db->query("SELECT COUNT(*) FROM ".TABLE." WHERE model_id = {$model_id} AND dil_id = '".LANGUAGE_DEFAULT."' ")->fetchColumn();
				$limit = 10;
										...
										<tr id="item-<?php echo $row["detay_ust_id"]; ?>" role="row" class="even"> 
											...
											<td class="sorting_1">
												<div class="d-flex">
													...
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=edit&id=<?php echo $row["detay_ust_id"]; ?>&detay&model_id=<?php echo $model_id ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>
													<a class="contact-icon" href="<?php echo AREA; ?>?do=delete&id=<?php echo $row["detay_ust_id"]; ?>&detay&model_id=<?php echo $model_id ?>"><i class="fas fa-trash-alt"></i></a>
												</div>
											</td>
										</tr> 
<?php 	
						...
						<?php if($toplam_veri_sayisi > $limit){ ?>
							<div class="dataTables_paginate paging_simple_numbers" id="example5_paginate">
								<?php 
										...
										if($sayfa > 1){	
											$onceki = $sayfa-1;	
											echo '<a href="?do=detay&model_id='.$model_id.'&q='.$onceki.'" class="paginate_button previous disabled">Geri</a>';
										}	
										if($sayfa==1){
											echo '<span><a class="paginate_button current" >1</a></span>';
										} else {
											echo '<span><a href="?do=detay&model_id='.$model_id.'&q=1" class="paginate_button previous">1</a></span>';
										}
										if($sayfa-$x > 2){
											echo '...'; 
											$i = $sayfa-$x;
										...
										for($i; $i<=$sayfa+$x; $i++) {
											if($i==$sayfa){
												...
												echo '<span><a href="?do=detay&model_id='.$model_id.'&q='.$i.'" class="paginate_button previous">'.$i.'</a></span>';
											}
											if($i==$sonSayfa) break;  
										}		
										if($sayfa < $sonSayfa){	  
											$sonraki = $sayfa+1;   
											echo '<a href="?do=detay&model_id='.$model_id.'&q='.$sonraki.'" class="paginate_button next disabled">İleri</a>';		  
										}	
								?> 
							</div>
						<?php } ?> 
