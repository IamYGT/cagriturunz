								...
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									<thead>
										...
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 115.6px;">Başlık</th>
										 <th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Company: activate to sort column ascending" style="width: 215.6px;">Dış Resim 400x600</th>
...
?>
										<tr id="item-<?php echo $row["egzoz_ust_id"]; ?>" role="row" class="even"> 
											...
											<td class=""><?php if(isset($row["egzoz_resim"])){ ?><img class="mr-3 " width="60" src="../uploads/exhaust/<?php echo $row["egzoz_resim"]; ?>" alt="<?php echo $row["haber_baslik"]; ?>"><?php }else{ ?><img class="mr-3 " width="60" src="images/no_images.png" alt="<?php echo $row["haber_baslik"]; ?>"><?php } ?></td>
...
<?php 
				...
				if(isset($submitControl)){
					if(!empty($_POST['egzoz_baslik']) || !empty($_POST['egzoz_aciklama'])) {
						...
						$upload = new Upload($_FILES['egzoz_resim2']);
						if ($upload->uploaded) { 
								$upload->file_auto_rename = true;
								$upload->process("../uploads/exhaust");
							if ($upload->processed){  
								$egzoz_resim2 = $upload->file_dst_name;
							} else { 
								$error = Bilgilendirme::Hata("Hay Aksi! Bir hata meydana geldi.".$upload->error);
							}
						}
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET egzoz_baslik = ?, egzoz_seo = ?, egzoz_aciklama = ?, egzoz_description = ?, egzoz_keywords = ?,  egzoz_resim = ?,  egzoz_resim2 = ?,  egzoz_header = ?,  egzoz_logo = ?,  dil_id = ?, egzoz_durum = ?, egzoz_ust_id = ?");
								$insert->execute(array($egzoz_baslik[$key], Seo_Link_Cevir($egzoz_baslik[$key]), $egzoz_aciklama[$key], $egzoz_description[$key], $egzoz_keywords[$key], $egzoz_resim, $egzoz_resim2, $egzoz_header, $egzoz_logo, $key, $egzoz_durum, $LastID));
					...
					<div class="col-xl-4 col-xxl-4">
                        <div class="card">
                            <div class="card-body p-3">
								<div class="form-group"> 
									<label for="">Dış Resim 400x600</label>
								...
								<div class="form-group"> 
									<label for="">İç Resim 1365x600</label>
									<small class="form-text text-muted">Seçmiş olduğunuz resim veri içeriğinde kullanılmaktadır.</small> 
									<div class="fileinput fileinput-new" data-provides="fileinput">
										<div class="fileinput-preview thumbnail" data-trigger="fileinput" style="width: 100%;height: 190px;line-height: 190px;">
											<img src="http://www.placehold.it/287x192/EFEFEF/AAAAAA&amp;text=Resim+Seç" class="img-fluid img-thumbnail" alt="">
										...
										<div>
											<span class="btn btn-primary btn-sm btn-file">
												<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
												<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
												<input type="file" name="egzoz_resim2" accept="image/*" id="egzoz_resim2">
											</span>
											<a href="#" class="btn btn-primary btn-sm fileinput-exists" data-dismiss="fileinput"><span class="fui-trash"></span>Vazgeç</a>
										</div>
									</div>
				...
				if ($row_check->rowCount() > 0) {
					if(isset($submitControl)){
						if(!empty($_POST['egzoz_baslik']) || !empty($_POST['egzoz_aciklama'])) {
							...
							$upload = new Upload($_FILES['egzoz_resim2']);  
							if(!isset($_FILES['egzoz_resim2']['name'])) {
								$egzoz_resim2 = null;
							} else if(!empty($_FILES['egzoz_resim2']['name'])) {
								$upload->file_auto_rename = true; 
								$upload->process("../uploads/exhaust");
								if ($upload->processed) {
									$egzoz_resim2 = $upload->file_dst_name;
								...
								$egzoz_resim2 = $image_info["egzoz_resim2"];
							}
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									...
									$update = $db->prepare("UPDATE ".TABLE." SET  egzoz_baslik = ?, egzoz_seo = ?, egzoz_aciklama = ?, egzoz_description = ?, egzoz_keywords = ?, egzoz_resim = ?, egzoz_resim2 = ?,   egzoz_durum = ? WHERE egzoz_ust_id = ? AND dil_id = ?");
									$update->execute(array($egzoz_baslik[$key], Seo_Link_Cevir($egzoz_baslik[$key]), $egzoz_aciklama[$key],  $egzoz_description[$key], $egzoz_keywords[$key], $egzoz_resim, $egzoz_resim2, $egzoz_durum, $id, $key));
								}
							}
							if ($update) {
					...
					<div class="col-xl-4 col-xxl-4">
                        <div class="card">
                            <div class="card-body p-3">
								...
								<div class="form-group"> 
								<label>İç Resim 1365x600</label>
								<small class="form-text text-muted">Seçmiş olduğunuz resim veri içeriğinde kullanılmaktadır.</small> 
								<?php if(empty($row_info["egzoz_resim2"])) {?>
										<div class="fileinput fileinput-new" data-provides="fileinput">
											<div class="fileinput-preview thumbnail" data-trigger="fileinput" style="width: 100%;height: 190px;line-height: 190px;">
												<img src="resim-sec.png" class="img-fluid img-thumbnail" alt="">
											</div> 
								...
								<?php }else{?> 
										<div class="fileinput fileinput-exists" data-provides="fileinput">
											...
											<div class="fileinput-preview fileinput-exists thumbnail" style="max-width: 303px; max-height: 190px; line-height: 190px;"><img src="../uploads/exhaust/<?php echo $row_info["egzoz_resim2"]; ?>" class="img-fluid img-thumbnail"></div>
											<div>
												<span class="btn btn-primary btn-sm btn-file">
													<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
													<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
													<input type="file" name="egzoz_resim2" id="egzoz_resim2" >
													<div class="ripple-container"></div>
												</span>
												<a href="#" class="btn btn-primary btn-sm fileinput-exists" data-dismiss="fileinput"><span class="fui-trash"></span>Vazgeç</a>
											</div>
