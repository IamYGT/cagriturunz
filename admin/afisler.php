...
<?php 		
			if($do == 'marka') {
				$sayfa = (isset($q) ? $q : 1);
				$toplam_veri_sayisi = $db->query("SELECT COUNT(*) FROM ".TABLE." WHERE egzoz_id = {$egzoz_id} AND  dil_id = '".LANGUAGE_DEFAULT."' ")->fetchColumn();
				$limit = 10;
							...
							<div id="example5_wrapper" class="dataTables_wrapper no-footer">
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									<thead>
										...
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 115.6px;">Proje</th>
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 115.6px;">Başlık</th>
											<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Company: activate to sort column ascending" style="width: 215.6px;">Resim 1188x547</th>
...
?>
										<tr id="item-<?php echo $row["marka_ust_id"]; ?>" role="row" class="even"> 
											...
											<td class=""><?php if(isset($row["marka_resim"])){ ?><img class="mr-3 " width="60" src="../uploads/exhaust/<?php echo $row["marka_resim"]; ?>" alt="<?php echo $row["haber_baslik"]; ?>"><?php }else{ ?><img class="mr-3 " width="60" src="images/no_images.png" alt="<?php echo $row["haber_baslik"]; ?>"><?php } ?></td>
											...
											<td class="sorting_1">
												<div class="d-flex"> 
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=edit&id=<?php echo $row["marka_ust_id"]; ?>&marka&egzoz_id=<?php echo $egzoz_id ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>
													<a class="contact-icon" href="<?php echo AREA; ?>?do=delete&id=<?php echo $row["marka_ust_id"]; ?>&marka&egzoz_id=<?php echo $egzoz_id ?>"><i class="fas fa-trash-alt"></i></a>
												</div>
											</td>
										</tr> 
<?php 	
						...
						<?php if($toplam_veri_sayisi > $limit){ ?>
							<div class="dataTables_paginate paging_simple_numbers" id="example5_paginate">
								<?php 
										...
										if($sayfa > 1){	
											$onceki = $sayfa-1;	
											echo '<a href="?do=marka&egzoz_id='.$egzoz_id.'&q='.$onceki.'" class="paginate_button previous disabled">Geri</a>';
										}	
										if($sayfa==1){
											echo '<span><a class="paginate_button current" >1</a></span>';
										} else {
											echo '<span><a href="?do=marka&egzoz_id='.$egzoz_id.'&q=1" class="paginate_button previous">1</a></span>';
										}
										if($sayfa-$x > 2){
											echo '...'; 
											$i = $sayfa-$x;
										...
										for($i; $i<=$sayfa+$x; $i++) {
											if($i==$sayfa){
												...
												echo '<span><a href="?do=marka&egzoz_id='.$egzoz_id.'&q='.$i.'" class="paginate_button previous">'.$i.'</a></span>';
											}
											if($i==$sonSayfa) break;  
										}		
										if($sayfa < $sonSayfa){	  
											$sonraki = $sayfa+1;   
											echo '<a href="?do=marka&egzoz_id='.$egzoz_id.'&q='.$sonraki.'" class="paginate_button next disabled">İleri</a>';		  
										}	
								?> 
							</div>
						<?php } ?> 
					...
					if(!empty($_POST['marka_baslik']) || !empty($_POST['marka_aciklama'])) {
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET marka_baslik = ?, marka_seo = ?, marka_aciklama = ?, marka_description = ?, marka_keywords = ?,  marka_resim = ?, dil_id = ?, marka_durum = ?, egzoz_id = ?, marka_ust_id = ?");
								$insert->execute(array($marka_baslik[$key], Seo_Link_Cevir($marka_baslik[$key]), $marka_aciklama[$key], $marka_description[$key], $marka_keywords[$key], $marka_resim, $key, $marka_durum, $egzoz_id, $LastID));
					...
					<div class="col-xl-8 col-xxl-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Afiş Ekle</h4>
                            </div>
                            <div class="card-body"> 
...
?>
				<form action="#" method="POST" enctype="multipart/form-data" id="form" style="display: inline-flex; "> 
					<div class="col-xl-8 col-xxl-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Afiş Düzenle</h4>
                            </div>
                            <div class="card-body"> 
								<div class="step-app" id="demo">
									<ul class="step-steps">
                            ...
                            <div class="card-body p-3">
								...
								<?php }else{?> 
										<div class="fileinput fileinput-exists" data-provides="fileinput">
											...
											<div class="fileinput-preview fileinput-exists thumbnail" style="max-width: 303px; max-height: 190px; line-height: 190px;"><img src="../uploads/exhaust/<?php echo $row_info["marka_resim"]; ?>" class="img-fluid img-thumbnail"></div>
											<div>
												<span class="btn btn-primary btn-sm btn-file">
													<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
													<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
