...
<?php header('Content-type: Application/xml; charset="utf8"', true); ?>
		<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
			...
			<url>
				<loc><?php echo $ayarlar["strURL"]; ?>/hakkinda</loc> 
				<changefreq>always</changefreq>
				<lastmod><?php echo date('c', strtotime(date("Y-m-d H:i:s"))); ?></lastmod>
				<priority>1</priority>
			</url>
				<url>
				<loc><?php echo $ayarlar["strURL"]; ?>/hizmetler-ve-danismanliklar</loc> 
				<changefreq>always</changefreq>
				<lastmod><?php echo date('c', strtotime(date("Y-m-d H:i:s"))); ?></lastmod>
				<priority>1</priority>
			</url>
...
<?php 
		//Markalar
		$list = $db->query("SELECT * FROM egzozlar WHERE egzoz_durum = 1 ORDER BY egzoz_tarih ASC");
		foreach($list as $row){  	 
			$strURL = $ayarlar["strURL"].'/hizmet/'.$row["egzoz_seo"]; 
?>
			<url>
...
<?php 
		...
		foreach($list as $row){  	 
			$strURL = $ayarlar["strURL"].'/blogdetay/'.$row["haber_seo"]; 
?>
			<url>
				<loc><?php echo $strURL ?></loc>
				<lastmod><?php echo date('c', strtotime(date("Y-m-d H:i:s"))); ?></lastmod>
...
<?php 
		...
		foreach($list as $row){  	 
			$strURL = $ayarlar["strURL"].'/proje/'.$row["haber_seo"]; 
?>
			<url>
				<loc><?php echo $strURL ?></loc>
				<lastmod><?php echo date('c', strtotime(date("Y-m-d H:i:s"))); ?></lastmod>
