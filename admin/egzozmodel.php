<?php require("../include/baglan.php");include("../include/fonksiyon.php"); 
		...
		define('TABLE',"egzoz_model");
		define('AREA',"egzozmodel");
		if(!isset($do)) $do = null; 
...
<?php 		
			if($do == 'model') {
				$sayfa = (isset($q) ? $q : 1);
				$toplam_veri_sayisi = $db->query("SELECT COUNT(*) FROM ".TABLE." WHERE marka_id = {$marka_id} AND dil_id = '".LANGUAGE_DEFAULT."' ")->fetchColumn();
				$limit = 10;
...
?>
					...
					<div style="margin-bottom: 15px;" class="col-xl-12">
						<a href="<?php echo AREA; ?>?do=add&model&marka_id=<?php echo $marka_id ?>"><button style="float: right;"  type="button" class="btn btn-rounded btn-primary"><span class="btn-icon-left text-primary"><i class="fa fa-plus color-primary"></i></span>Yeni</button></a>
					</div>
					<div class="col-xl-12"> 
						<div class="table-responsive"> 
							<div id="example5_wrapper" class="dataTables_wrapper no-footer">
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									...
									<tbody id="sortable"> 
<?php
									$list = $db->query("SELECT * FROM ".TABLE." WHERE marka_id = {$marka_id} AND dil_id = '".LANGUAGE_DEFAULT."' ORDER BY row ASC LIMIT $baslangic,$limit"); 
										if ($list->rowCount()){
											foreach($list as $row){  
											$egzozmarka_info = $db->query("SELECT * FROM egzoz_marka WHERE marka_ust_id = {$marka_id}")->fetch(PDO::FETCH_ASSOC);
											$egzoz_info = $db->query("SELECT * FROM egzozlar WHERE egzoz_ust_id = {$egzozmarka_info["egzoz_id"]}")->fetch(PDO::FETCH_ASSOC);
?>
										<tr id="item-<?php echo $row["model_ust_id"]; ?>" role="row" class="even"> 
											...
											<td class=""><?php echo $row["model_baslik"]; ?></td>
											<td class=""><?php echo ($row["model_durum"] == 1 ? '<span class="badge badge-rounded badge-secondary">Aktif</span>' : '<span class="badge badge-rounded badge-danger">Pasif</span>'); ?></td>
											<td class="sorting_1">
												<div class="d-flex">
													<a class="contact-icon mr-3" href="egzozdetay?do=detay&model_id=<?php echo $row["model_ust_id"]; ?>"><i class="fas fa-link" aria-hidden="true"></i></a>
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=edit&id=<?php echo $row["model_ust_id"]; ?>&model&marka_id=<?php echo $marka_id ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>
													<a class="contact-icon" href="<?php echo AREA; ?>?do=delete&id=<?php echo $row["model_ust_id"]; ?>&model&marka_id=<?php echo $marka_id ?>"><i class="fas fa-trash-alt"></i></a>
												</div>
											</td>
										</tr> 
<?php 	
						...
						<?php if($toplam_veri_sayisi > $limit){ ?>
							<div class="dataTables_paginate paging_simple_numbers" id="example5_paginate">
								<?php 
										...
										if($sayfa > 1){	
											$onceki = $sayfa-1;	
											echo '<a href="?do=model&marka_id='.$marka_id.'&q='.$onceki.'" class="paginate_button previous disabled">Geri</a>';
										}	
										if($sayfa==1){
											echo '<span><a class="paginate_button current" >1</a></span>';
										} else {
											echo '<span><a href="?do=model&marka_id='.$marka_id.'&q=1" class="paginate_button previous">1</a></span>';
										}
										if($sayfa-$x > 2){
											echo '...'; 
											$i = $sayfa-$x;
										...
										for($i; $i<=$sayfa+$x; $i++) {
											if($i==$sayfa){
												...
												echo '<span><a href="?do=model&marka_id='.$marka_id.'&q='.$i.'" class="paginate_button previous">'.$i.'</a></span>';
											}
											if($i==$sonSayfa) break;  
										}		
										if($sayfa < $sonSayfa){	  
											$sonraki = $sayfa+1;   
											echo '<a href="?do=model&marka_id='.$marka_id.'&q='.$sonraki.'" class="paginate_button next disabled">İleri</a>';		  
										}	
								?> 
							</div>
						<?php } ?> 
...
<?php 
				...
				if(isset($submitControl)){
					if(!empty($_POST['model_baslik']) || !empty($_POST['model_aciklama'])) {
						$LastID = $db->query("SELECT model_id FROM ".TABLE." ORDER BY model_id DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
						$LastID = $LastID["model_id"]+1;
						foreach ($_POST AS $k=>$v) {
							$v = $v;
							if (substr($k,0,5) == "form_") {
								$key = str_replace("form_","",$k);
								$insert = $db->prepare("INSERT INTO ".TABLE." SET model_baslik = ?, model_seo = ?, model_aciklama = ?, model_description = ?, model_keywords = ?, dil_id = ?, model_durum = ?, marka_id = ?, model_ust_id = ?");
								$insert->execute(array($model_baslik[$key], Seo_Link_Cevir($model_baslik[$key]), $model_aciklama[$key], $model_description[$key], $model_keywords[$key], $key, $model_durum, $marka_id, $LastID));
													...
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																...
																<input type="text" name="model_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Başlık" required="">
															</div>
														</div>  
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Description</label>
																<input type="text" name="model_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Description" required="">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Keywords</label>
																<input type="text" name="model_keywords[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Keywords" required="">
															</div>
														</div>  
														...
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																...
																<textarea class="form-control ckeditor" name="model_aciklama[<?php echo $LANGUAGE_CODE; ?>]"></textarea>
															</div>
														</div>
													</div>
...
<?php 
				...
				$row_check = $db->prepare("SELECT * FROM ".TABLE." WHERE model_ust_id = ?");
				...
				if ($row_check->rowCount() > 0) {
							...
							if ($update) {
								foreach ($_POST AS $k=>$v) {
									...
									if (substr($k,0,5) == "form_") {
										...
										$update = $db->prepare("UPDATE ".TABLE." SET  model_baslik = ?, model_seo = ?, model_aciklama = ?, model_description = ?, model_keywords = ?, model_durum = ? WHERE model_ust_id = ? AND dil_id = ?");
										$update->execute(array($model_baslik[$key], Seo_Link_Cevir($model_baslik[$key]), $model_aciklama[$key],  $model_description[$key], $model_keywords[$key], $model_durum, $id, $key));
									}
								}
								echo '<meta http-equiv="refresh" content="1;url='.AREA.'?do=model&marka_id='.$marka_id.'">';
					...
					$row_info = $db->query("SELECT * FROM ".TABLE." WHERE model_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
?>
				<form action="#" method="POST" enctype="multipart/form-data" id="form" style="display: inline-flex; "> 
					<div class="col-xl-8 col-xxl-8">
                        <div class="card">
												...
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Başlık</label>
																<input type="text" name="model_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Başlık" required="" value="<?php echo GetTableValue("model_baslik",TABLE,"where model_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Description</label>
																<input type="text" name="model_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Description" required="" value="<?php echo GetTableValue("model_description",TABLE,"where model_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Keywords</label>
																<input type="text" name="model_keywords[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Keywords" required="" value="<?php echo GetTableValue("model_keywords",TABLE,"where model_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div> 
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Açıklama</label>
																<textarea class="form-control ckeditor" name="model_aciklama[<?php echo $LANGUAGE_CODE; ?>]"><?php echo GetTableValue("model_aciklama",TABLE,"where model_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?></textarea>
															</div>
														</div>
													</div>
													<input type="hidden" name="form_<?php echo $LANGUAGE_CODE; ?>" value="<?php echo $LANGUAGE_CODE; ?>" />
...
<?php 
				...
				$row_check->execute(array($id)); 
				if ($row_check->rowCount() > 0) {
					$update = $db->exec("DELETE FROM ".TABLE." WHERE model_ust_id = {$id};");
					if($update) {
						echo '<meta http-equiv="refresh" content="1;url='.AREA.'?do=model&marka_id='.$marka_id.'">';
