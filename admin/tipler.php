						...
						<?php if($toplam_veri_sayisi > $limit){ ?>
							<div class="dataTables_paginate paging_simple_numbers" id="example5_paginate">
								<?php 
										...
										if($sayfa > 1){	
											$onceki = $sayfa-1;	
											echo '<a href="?yil&yil_id='.$yil_id.'&q='.$onceki.'" class="paginate_button previous disabled">Geri</a>';
										}	
										if($sayfa==1){
											echo '<span><a class="paginate_button current" >1</a></span>';
										} else {
											echo '<span><a href="?yil&yil_id='.$yil_id.'&q=1" class="paginate_button previous">1</a></span>';
										}
										if($sayfa-$x > 2){
											echo '...'; 
											$i = $sayfa-$x;
										...
										for($i; $i<=$sayfa+$x; $i++) {
											if($i==$sayfa){
												...
												echo '<span><a href="?yil&yil_id='.$yil_id.'&q='.$i.'" class="paginate_button previous">'.$i.'</a></span>';
											}
											if($i==$sonSayfa) break;  
										}		
										if($sayfa < $sonSayfa){	  
											$sonraki = $sayfa+1;   
											echo '<a href="?yil&yil_id='.$yil_id.'&q='.$sonraki.'" class="paginate_button next disabled">İleri</a>';		  
										}	
								?> 
							</div>
						<?php } ?> 
...
<?php 
				...
				if ($row_check->rowCount() > 0) {
					if(isset($submitControl)){
						if(!empty($_POST['arac'])) {  
							$update = $db->prepare("UPDATE ".TABLE." SET yil = ?, arac = ?, link = ?, oguc = ?, yguc = ?, gfark = ?, gyuz = ?, otork = ?, ytork = ?, tfark = ?, tyuz = ?, ytas = ?, yakit = ?, tip_durum = ?,  arac_id = ?");
